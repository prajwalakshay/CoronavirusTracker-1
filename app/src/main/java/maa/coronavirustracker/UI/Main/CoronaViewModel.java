        ...
        CoronaClient.getCoronaClient().getCoronaResumeInformation().enqueue(new Callback<Resume>() {
            ...
            public void onResponse(@NonNull Call<Resume> call, @NonNull Response<Resume> response) {
                if (response.body() != null) {
                    mutableResumeLiveData.setValue(new Resume(response.body().getRecovered(), response.body().getCases(), response.body().getDeaths(),response.body().getTodayCases(),response.body().getTodayDeaths(),response.body().getActive()));
                }
            }
